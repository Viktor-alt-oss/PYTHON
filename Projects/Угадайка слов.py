...
world_list = ['год', 'человек', 'время', 'дело', 'жизнь', 'день', 'рука', 'раз', 'работа', 'слово', 'место', 'лицо',
...
def is_valid(n):
    while not n.isalpha():
        n = input('Цифры и символы вводить нельзя, введите букву: ')
    return True
...
def play():
    ...
    while True:
        ...
        print('на данный момент слово выглядит так ', word_completion)
        
        s = input('Введи букву загаданного числа: ').upper()
        while s in guessed_letters:
            s = input("Вы уже вводили данную букву, введите другую  ")  
            continue        
...
def play_return():
    while True:
        n = input('желаете еще сыграть? Ответьте "да" или "нет":  ')
        while n != 'да' and n != 'нет':
            n = input("Введите корректно: ")
            continue  
        if n == 'нет':
